...
const FormEmotion = () => {
  const [emotions, setEmotions] = useState(
    Object.keys(EMOTION_LABELS).reduce(
      (acc, key) => ({
        ...acc,
        [key]: {
          checked: false,
          value: key === "happiness" ? 10 : key === "surprise" ? 0 : -8,
        },
      }),
      {}
    )
  ...
  const [secondaryEmotions, setSecondaryEmotions] = useState(
    Object.values(SECONDARY_EMOTIONS_MAP)
      .flat()
      .reduce(
        (acc, emotion) => ({
          ...acc,
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const selectedEmotion = Object.entries(emotions).find(
      ([_, emotion]) => emotion.checked
    );
    if (selectedEmotion) {
      try {
        const selectedSecondary = Object.entries(secondaryEmotions).find(
          ([_, emotion]) => emotion.checked
        )?.[0];
        const emotionData = {
          id: null,
    ...
    >
      ...
      >
        <form onSubmit={handleSubmit} className="emotion-form">
          <h2 style={{ color: "#5E1151", textAlign: "center" }}>
            Mis Emociones
          </h2>
          <div
            style={{
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  Quieres comentarnos el porque?(o si prefieres escribe que no
                  te sientes comodo escribiendolo):
                </label>
                <input
                  type="text"
                  value={details}
            ...
            <div style={modalStyles.overlay}>
              <div style={modalStyles.content}>
                <h3 style={{ textAlign: "center" }}>
                  ¿Cuál se acerca más a lo que sientes?
                </h3>
                <div
                  style={{
                    backgroundColor: "#F0EDFA",
                ...
                >
                  {SECONDARY_EMOTIONS_MAP[selectedPrimaryEmotion].map(
                    (option) => (
                      <label
                        key={option}
                        className="emotion-label"
